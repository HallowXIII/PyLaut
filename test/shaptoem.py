...
def is_sibilant(seg):
    return (seg.feature_is_true("sibilant"))
...
def get_changes():
    ...
    zsr = Change().do(lambda x: Phoneme("m")).to(
        This.forall(Phone)(lambda p: p.is_symbol("p"))).when(
            This.at(Phone, 1, lambda p: is_sibilant(p)))

    mt = Change().do(lambda this: metathesize(
        this.syllable, this.syllable.phonemes.index(this.syllable.get_nucleus()[0]), -1)).to(
    ...
    devoice_z = Change().do(
        lambda this: change_feature(this.phoneme, "voice", False)).to(
            This.forall(Phone)(lambda p: p.is_symbol("z")))

    h_lengthen1 = Change().do(
        lambda this: change_feature(this.phoneme, "long", True)).to(
            This.forall(Phone)(lambda p: p.is_vowel())).when(
                This.at(Phone, -1, lambda c: c.is_symbol("h")))

    h_lengthen2 = Change().do(
        lambda this: change_feature(this.phoneme, "long", True)).to(
            This.forall(Phone)(lambda p: p.is_vowel())).when(
                This.at(Phone, 1, lambda c: c.is_symbol("h")))

    htoa1 = Change().do(lambda p: Phoneme("aː")).to(
        This.forall(Phone)(lambda p: p.is_symbol("h"))).when(
            This.at(Phone, -1, lambda c: c.is_consonant())).when(
                This.at(Phone, 1, lambda c: c.is_consonant()))

    htoa2 = Change().do(lambda p: Phoneme("aː")).to(
        This.forall(Phone)(lambda p: p.is_symbol("h"))).when(
            This.at(Phone, -1, lambda c: c.is_consonant())).when(
                This.is_at_index(Phone, -1))

    delete_h = Change().do(lambda p: []).to(
        This.forall(Phone)(lambda p: p.is_symbol("h")))

    voice_f = Change().do(lambda p: Phoneme("v")).to(
        This.forall(Phone)(lambda p: p.is_symbol("f"))).when(
            This.at(Phone, -1, lambda v: v.is_vowel())).when(
                This.at(Phone, 1, lambda v: v.is_vowel()))

    thtoz = Change().do(lambda p: Phoneme("z")).to(
        This.forall(Phone)(lambda p: p.is_symbol("θ")))

    changes = [
        zsr, mt, vlength, shtosn, devoice_z, h_lengthen1, h_lengthen2, htoa1,
        htoa2, delete_h, voice_f, thtoz
    ]
